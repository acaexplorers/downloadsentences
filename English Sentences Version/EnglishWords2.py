...
def main():
    ...
    def download_audio(word,id,sent,write_list):

        if '0' in word:
            word = word.replace('0','')
        if len(write_list)<1:
            write_list.append(word)
        url = 'http://audio.tatoeba.org/sentences/eng/'+id+'.mp3'
        try:
        ...
        try:
            write_list.append(ids[index][2])
            write_list.append(ids[index][2].replace(ids[index][0].lower(),'_____'))
    ...
    SITE_URL = 'https://tatoeba.org/eng/sentences/search?from=eng&to=spa&query={}'
